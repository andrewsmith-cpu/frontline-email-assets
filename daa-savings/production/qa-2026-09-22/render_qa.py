...
records=[
 dict(client_id='Z5537604',name='Hope Trujillo',review_date='09/16/2026',debt=39182,current=1175.46,do_nothing=83849.48,program_payment=590.41,program_term=60,monthly_savings=585.05,total_savings=48425.03,p24=1465.08,p36=978.46,p48=736.70,p54=655.21,p60=590.41),
 dict(client_id='Z3727927',name='Jeffrey Rawlins',review_date='09/18/2026',debt=42868,current=1286.04,do_nothing=91737.52,program_payment=645.95,program_term=60,monthly_savings=640.09,total_savings=52980.56,p24=1602.91,p36=1070.51,p48=806.01,p54=716.85,p60=645.95),
 dict(client_id='Z5699916',name='William Hurst',review_date='09/14/2026',debt=102417,current=3968.00,do_nothing=219172.38,program_payment=1539.69,program_term=60,monthly_savings=2428.31,total_savings=126790.98,p24=3833.10,p36=2558.99,p48=1921.93,p54=1709.57,p60=1539.69),
 dict(client_id='Z2492462',name='Curtis Page',review_date='08/10/2026',debt=167773,current=8577.00,do_nothing=359034.22,program_payment=2528.06,program_term=60,monthly_savings=6048.94,total_savings=207350.62,p24=6320.15,p36=4213.43,p48=3160.07,p54=2808.95,p60=2528.06),
]
...
def value_size(v,normal,small):
    return small if v is not None and abs(v)>=100000 else normal
def payment_size(v):
    return 30 if v is None else (34 if abs(v)>=10000 else 40)
def monthly_savings_size(v):
    return 43 if v is not None and abs(v)>=1000 else 52

def render(row):
    name=row['name']; cid=row['client_id']; date=row['review_date']; term=int(row['program_term'])
    vals={k:row[k] for k in ['debt','current','do_nothing','program_payment','monthly_savings','total_savings','p24','p36','p48','p54','p60']}
    with tempfile.TemporaryDirectory() as td0:
        td=Path(td0)
        base=Image.open(PROOF).convert('RGB').resize((1055,1491),Image.Resampling.LANCZOS)
        assets={'header':(0,0,1055,253),'footer':(0,1090,1055,1400),'current':(36,432,102,500),'nothing':(289,432,356,500),'program':(548,432,616,500),'savings':(801,432,868,500),'debt':(596,322,662,394),'pig':(620,978,701,1065)}
        uris={}
        for key,box in assets.items():
            p=td/f'{key}.png'; base.crop(box).save(p); uris[key]=p.as_uri()
        terms=[24,36,48,54,60]
        pv=[vals['p24'],vals['p36'],vals['p48'],vals['p54'],vals['p60']]
        pay_html=''.join(f'''<div class="pay {'sel' if t==term else ''}"><div class="term">{t} MONTHS</div><div class="payval" style="font-size:{payment_size(v)}px">{html.escape(money(v))}</div></div>''' for t,v in zip(terms,pv))
...
<div class="card"><div class="card-top"><img src="{uris['savings']}"><div class="card-title">ESTIMATED<br>MONTHLY SAVINGS</div></div><div class="sub">You Could Save<br>Each Month</div><div class="rule"></div><div class="big" style="font-size:{monthly_savings_size(vals['monthly_savings'])}px">{html.escape(money(vals['monthly_savings']))}</div></div>
</section>
...
<img class="footer" src="{uris['footer']}"></div></body></html>'''
        ...
        png=td/'out.png'; pix.save(png)
        img=Image.open(png).convert('RGB')
        if img.size!=(1055,1400):
            img=img.resize((1055,1400),Image.Resampling.LANCZOS)
        out=OUT/f"{safe(cid)}_{safe(name)}_DAA_GENIE_QA.jpg"
        img.save(out,quality=91,optimize=True,progressive=True)
        print(out)
